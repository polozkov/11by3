...
SVG.DRAW = {
  ...
  f_circle: function(nxy) {
    let cxy = nxy.f_op_add_same(0.5);
    let svg = '<circle ';
    svg += 'cx="' + cxy.x + '" ';
    svg += 'cy="' + cxy.y + '" ';
  ...
  f_arrow: function(nxy, dir_xy) {
    ...
    return SVG.DRAW.f_line(c, c_end) + SVG.DRAW.f_line(a_end, c_end) + SVG.DRAW.f_line(b_end, c_end);
  },

  //рисуй клетку (is_same_stroke_fill = false, значит чёрная граница, или серая)
  f_rect_cell: function(nxy, n_color_fill, is_gray = false) {
    ...
    return SVG.DRAW.f_rect(nxy, color_fill, CONST_1_1, color_stroke_border);
  },

  //рисуй уголок (is_same_stroke_fill значит "нужна ли чёрная обводка" или пойдёт серая)
  f_polyomino: function(i_polyomino, is_gray = false) {
    let svg_result = "";
    for (let i_nxy of i_polyomino.arr_xy)
      svg_result += SVG.DRAW.f_rect_cell(i_nxy, i_polyomino.arr_xy.length, is_gray);
    ...
    if (i_polyomino.f_is_reflex_mark_direction(i)) {
      //кружочек
      svg_result += SVG.DRAW.f_circle(i_polyomino.arr_xy[i]);
      //а где кружочек, там обязательно и стрелка в нужном направлении
      svg_result += SVG.DRAW.f_arrow(i_polyomino.arr_xy[i], i_polyomino.f_is_reflex_mark_direction(i));
    }
  ...
  //в итоге рисуем всё в зависимости от настроек (obj_special_corner - замени уголок на прорисовке на особый)
  f_final: function(obj_special_corner = null, setting = SVG.SETTING) {
    let svg_corners_and_board = "";
    //пустое клетчатое поле (пустые клеточки)
    svg_corners_and_board += SVG.DRAW.f_empty_grid(setting.nxy_answer_from, setting.nxy_answer_sizes);
    ...
    for (let n_always_slots of [0,1,2,3,4,5]) {
      ...
      svg_corners_and_board += SVG.DRAW.f_polyomino(i_polyomino, true);
    }

    //рисуй полимино с границами (если в зоне старта, то поверх "домиков-слотов")
    for (let i_polyomino of setting.arr_corners) {
      let svg_polyomino = SVG.DRAW.f_polyomino(i_polyomino, false);
      //замени уголок на прорисовке на особый (а на старом месте тот, который заменяют, не рисуй)
      if ((obj_special_corner != null) && (obj_special_corner.arr_xy.length == i_polyomino.arr_xy.length)) {
        svg_polyomino = SVG.DRAW.f_polyomino(obj_special_corner, false);
      }
    ...
    svg_corners_and_board += SVG.DRAW.f_rect(setting.nxy_answer_from, "none", setting.nxy_answer_sizes);
    svg_corners_and_board += SVG.DRAW.f_rect(setting.nxy_start_from, "none", setting.nxy_start_sizes);
